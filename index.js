...
const SUPPORT_USERNAME = '@adiler9227'; // Твой username для поддержки

// Кэш последних вопросов пользователям (чтобы не повторяться)
...
bot.on('text', async (ctx) => {
  ...
  try {
    ...
    if (aiDecision.action === 'record') {
      // Очищаем кэш вопросов для этого пользователя
      userLastQuestion.delete(userId);
      ...
      const currentQuestion = aiDecision.message;
      
      if (lastQuestion === currentQuestion) {
        // Уже спрашивали - не понимаем, даем подсказку
        await ctx.reply(
          `🤷 Не совсем понял что записать.\n\n` +
          `Попробуй написать так:\n` +
          `📝 "принял заказ 100 боди от Анны"\n` +
          `📝 "сшил 50 штук для Сайакал"\n` +
          `📝 "отгрузил Маркову 200"\n\n` +
          `Или напиши /help для помощи`
        );
        userLastQuestion.delete(userId);
      } else {
        // Первый раз спрашиваем - сохраняем вопрос
        userLastQuestion.set(userId, currentQuestion);
    ...
    // Детальное сообщение об ошибке с контактом поддержки
    let errorMsg = '❌ *Произошла ошибка!*\n\n';
    
    if (error.message && error.message.includes('OpenAI')) {
      errorMsg += '🤖 Проблема с AI обработкой.\n\n';
    } else if (error.message && error.message.includes('Supabase')) {
      errorMsg += '💾 Проблема с базой данных.\n\n';
    } else {
      errorMsg += '⚠️ Внутренняя ошибка системы.\n\n';
    }
    
    errorMsg += `Попробуй еще раз или напиши в поддержку:\n`;
    errorMsg += `👤 ${SUPPORT_USERNAME}\n\n`;
    errorMsg += `_Твой ID: ${userId}_\n`;
    errorMsg += `_Сообщение: "${text}"_`;
...
📝 Работа - уборка, починка, организация, встреча, звонок, документы, ЛЮБАЯ другая работа

ПОНИМАЙ ЛЮБУЮ РАБОТУ:
"сделал уборку в офисе" = 📝 Работа, детали: "уборка в офисе"
"починил оборудование" = 📝 Работа, детали: "починил оборудование"  
"организовал доставку" = 📝 Работа, детали: "организовал доставку"
"провел встречу с клиентом" = 📝 Работа, детали: "встреча с клиентом"

ЕСЛИ РАБОТА НЕ СВЯЗАНА С КЛИЕНТАМИ:
- Записывай БЕЗ клиента!
- Не спрашивай про клиента если это офисная/техническая работа!
- Примеры БЕЗ клиента: уборка, ремонт, документы, организация
...
ПРАВИЛА УТОЧНЕНИЯ:
- Если есть работа И клиент И количество → СРАЗУ record!
- Если есть работа БЕЗ клиента И это офисная работа (уборка, ремонт и т.д.) → СРАЗУ record БЕЗ клиента!
- Если производственная работа (пошив, заказ, отгрузка) БЕЗ клиента → "Для какого клиента?"
- Если непонятно что за работа → "Какую работу сделал?"
- Если нет количества для производства → записать БЕЗ количества (необязательно)
- НЕ задавай ОДИНАКОВЫЕ вопросы дважды!
- НЕ уточняй если информация полная!
